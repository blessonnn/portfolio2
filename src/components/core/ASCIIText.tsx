...
class CanvAscii {
  ...
  render() {
    ...
    if (this.mesh && this.mesh.material && "uniforms" in this.mesh.material) {
      const uniforms = this.mesh.material.uniforms as { [key: string]: { value: any } };
      uniforms.uTime.value = Math.sin(time);
    }
      ...
      if (mesh.isMesh && mesh.material) {
        if (Array.isArray(mesh.material)) {
          ...
          Object.keys(mesh.material).forEach((key) => {
            const prop = (mesh.material as any)[key];
            if (prop && typeof prop.dispose === "function") {
              prop.dispose();
            }
          });
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const startInstance = async (container: HTMLDivElement, w: number, h: number) => {
      asciiRef.current = await createAndInit(container, w, h);
      if (!cancelled && asciiRef.current) {
        // Setup visibility observer to play/pause loop
    ...
    const setup = async () => {
          ...
          async ([entry]) => {
            ...
            if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
              ...
              if (!cancelled && containerRef.current) {
                await startInstance(containerRef.current, w, h);
              }
            }
          },
          { threshold: 0.1 }
      ...
      await startInstance(containerRef.current, width, height);
    };
