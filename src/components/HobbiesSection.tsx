...
// PLAYLIST_DATA moved to PlaylistView.tsx
      ...
      {activeHobby !== null && typeof window !== "undefined" && createPortal(
        ...
        >
          <div ref={gridContentRef} className="w-full h-full flex flex-col overflow-hidden relative">
            {/* Header / Back Button */}
            <div className="w-full max-w-7xl mx-auto px-4 md:px-12 pt-8 pb-4 flex items-center justify-between z-50 shrink-0">
              <button
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleCloseGrid(); }}
                className={`px-6 py-2 text-xl md:text-2xl font-medium rounded-full transition-colors flex items-center gap-2 cursor-pointer cursor-target shadow-2xl ${
                  activeHobby === "playlist" ? "bg-white text-black hover:bg-white/80" : "bg-black text-white hover:bg-black/80"
                }`}
              >
                &larr; Back
              </button>
              <h3 className={`text-2xl md:text-3xl font-bold uppercase tracking-widest ${
                activeHobby === "playlist" ? "text-white" : "text-black"
              }`}>
